...
def backtest(df: pd.DataFrame, tenkan_period: int, kijun_period: int):
    ...
    df["signal"] = np.where((df["tenkan_minus_kijun"] > 0) & (df["prev_tenkan_minus_kijun"] < 0) &
                            (df["close"] > df["senkou_span_a"]) & (df["close"] > df["senkou_span_b"]) &
                            (df["close"] > df["chikou_span"]), 1,

                            np.where((df["tenkan_minus_kijun"] < 0) & (df["prev_tenkan_minus_kijun"] > 0) &
                            (df["close"] < df["senkou_span_a"]) & (df["close"] < df["senkou_span_b"]) &
                            (df["close"] < df["chikou_span"]), -1, 0))

    signal_data = df[df["signal"] != 0].copy()
    df["pnl"] = signal_data["close"].pct_change() * signal_data["signal"].shift(1)
